Shader "Skill0/Particles/WindSwordSlash"
{
	Properties
	{
		...
		_TintColor("Tint Color", Color) = (0.7,1,0.88,0.72)
		...
		SubShader
		{
			...
			Pass {			
				...
				fixed4 frag ( v2f i  ) : SV_Target
				{
					...
					float slashLuminance = dot( clampResult37, float3( 0.299, 0.587, 0.114 ) );
					float3 tintedSlash = slashLuminance * _TintColor.rgb;
					float4 appendResult3 = (float4(( ( _AddColor * i.color ) + ( _Emission * float4( tintedSlash , 0.0 ) * i.color ) ).rgb , ( i.color.a * clampResult6 * ifLocalVar23 * _TintColor.a )));		
